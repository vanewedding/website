import wedding from "/img/wedding/wedding-1.jpg";
import welcome from "/img/welcome/welcome-1.jpg";
import gender from "/img/gender/gender-1.jpg";
import baby from "/img/baby/baby-1.jpg";
import baptism from "/img/baptism/baptism-1.jpg";
import communion from "/img/comunioni/comunioni-1.jpg";
import confirmation from "/img/cresime/cresime-1.jpg";
import bday from "/img/18esimi/18esimo-1.jpg";
import graduation from "/img/graduation/graduation-1.jpg";
import party from "/img/party/party-1.jpg";
import showcases from "/img/vetrine/vetrine-1.jpg";
import { LINK_TYPE } from "../constants/enum";
export const helmetData = {
	title: "Portfolio Matrimoni ed Eventi in Sicilia | Vanè",
	description:
		"Esplora matrimoni, destination wedding, cerimonie ed eventi curati da Vanè a Pozzallo, Ragusa e in Sicilia.",
	keywords:
		"portfolio matrimoni Sicilia, gallery matrimoni Pozzallo, wedding design Ragusa, destination wedding Sicilia, eventi Pozzallo, eventi Ragusa, allestimenti matrimonio Sicilia, cerimonie Sicilia",
	ogtitle: "Portfolio Matrimoni ed Eventi in Sicilia | Vanè",
	ogtype: "website",
	ogdescription:
		"Gallery di matrimoni, destination wedding, cerimonie ed eventi progettati in Sicilia.",
	ogurl: "https://www.vanewedding.it/#/it/gallery",
	ogimage: "https://www.vanewedding.it/img/gallery1.jpg",
};
export const galleryData = {
	title: {
		it: "GALLERIA",
		eng: "GALLERY",
	},
	album: [
		{
			id: 1,
			line1: {
				mobile: {
					it: "WEDDING",
					eng: "WEDDING",
				},
				desktop: {
					it: "WEDDING",
					eng: "WEDDING",
				},
			},
			images: [
				{
					id: 1,
					src: wedding,
					alt: {
						it: "Anello di fidanzamento",
						eng: "Engagement ring",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "wedding",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 2,
			line1: {
				mobile: {
					it: "GENDER REVEAL",
					eng: "GENDER REVEAL",
				},
				desktop: {
					it: "GENDER REVEAL",
					eng: "GENDER REVEAL",
				},
			},
			images: [
				{
					id: 1,
					src: gender,
					alt: {
						it: "Biscotti, bocce di confetti e palloncini rosa e azzurri",
						eng: "Pink and blue cookies, candy bowls and balloons",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "gender-reveal",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 3,
			line1: {
				mobile: {
					it: "BABY SHOWER",
					eng: "BABY SHOWER",
				},
				desktop: {
					it: "BABY SHOWER",
					eng: "BABY SHOWER",
				},
			},
			images: [
				{
					id: 1,
					src: baby,
					alt: {
						it: "Tavolo di legno con dolcetti, palloncini bianchi e beige e peluche di orso",
						eng: "Wooden table with sweets, white and beige balloons, and teddy bear plush",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "baby-shower",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 4,
			line1: {
				mobile: {
					it: "WELCOME HOME",
					eng: "WELCOME HOME",
				},
				desktop: {
					it: "WELCOME HOME",
					eng: "WELCOME HOME",
				},
			},
			images: [
				{
					id: 1,
					src: welcome,
					alt: {
						it: "Bocce di confetti con cartellino che indica i gusti",
						eng: "Candy bowls with tags indicating the flavors",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "welcome-home",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 5,
			line1: {
				mobile: {
					it: "BATTESIMO",
					eng: "BAPTISM",
				},
				desktop: {
					it: "BATTESIMO",
					eng: "BAPTISM",
				},
			},
			images: [
				{
					id: 1,
					src: baptism,
					alt: {
						it: "Dolcetti e confetti a tema coniglietti",
						eng: "Bunny-themed sweets and candies",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "baptism",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 6,
			line1: {
				mobile: {
					it: "PRIMA COMUNIONE",
					eng: "FIRST COMMUNION",
				},
				desktop: {
					it: "PRIMA COMUNIONE",
					eng: "FIRST COMMUNION",
				},
			},
			images: [
				{
					id: 1,
					src: communion,
					alt: {
						it: "Allestimento esterno con fiori rosa",
						eng: "Outdoor setup with pink flowers",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "communion",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 7,
			line1: {
				mobile: {
					it: "CRESIMA",
					eng: "CONFIRMATION",
				},
				desktop: {
					it: "CRESIMA",
					eng: "CONFIRMATION",
				},
			},
			images: [
				{
					id: 1,
					src: confirmation,
					alt: {
						it: "Scritta con nome del ragazzo sulla torta",
						eng: "Sign with the boy's name on the cake",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "confirmation",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 8,
			line1: {
				mobile: {
					it: "DICIOTTESIMO",
					eng: "18TH BIRTHDAY",
				},
				desktop: {
					it: "DICIOTTESIMO",
					eng: "18TH BIRTHDAY",
				},
			},
			images: [
				{
					id: 1,
					src: bday,
					alt: {
						it: "Festeggiata che spezza la candelina del diciottesimo compleanno",
						eng: "Birthday girl breaking the candle of her 18th birthday cake",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "18th-birthday",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 9,
			line1: {
				mobile: {
					it: "LAUREA",
					eng: "GRADUATION",
				},
				desktop: {
					it: "LAUREA",
					eng: "GRADUATION",
				},
			},
			images: [
				{
					id: 1,
					src: graduation,
					alt: {
						it: "Arco dorato con scritta 'Laurea', lucine e drappeggio rosso",
						eng: "Golden arch with 'Graduation' sign, fairy lights and red drapery",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "graduation",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 10,
			line1: {
				mobile: {
					it: "PARTY",
					eng: "PARTY",
				},
				desktop: {
					it: "PARTY",
					eng: "PARTY",
				},
			},
			images: [
				{
					id: 1,
					src: party,
					alt: {
						it: "Festeggiata avvolta da una pioggia di lucine",
						eng: "Birthday girl surrounded by a shower of fairy lights",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "party",
				linkType: LINK_TYPE.SLUG,
			},
		},
		{
			id: 11,
			line1: {
				mobile: {
					it: "VETRINE",
					eng: "SHOWCASES",
				},
				desktop: {
					it: "VETRINE",
					eng: "SHOWCASES",
				},
			},
			images: [
				{
					id: 1,
					src: showcases,
					alt: {
						it: "Vetrina di un fotografo decorata con cerchi dorati e nebbiolina",
						eng: "Photographer's showcase decorated with golden rings and baby-breath",
					},
				},
			],
			buttonData: {
				it: "GUARDA LE FOTO",
				eng: "WATCH THE ALBUM",
				url: "showcase",
				linkType: LINK_TYPE.SLUG,
			},
		},
	],
};

const weddingAltTexts = [
	{ it: "Anello di fidanzamento", eng: "Engagement ring" },
	{
		it: "Tavolo degli sposi vista mare",
		eng: "Bride and groom table with sea view",
	},
	{
		it: "Segnaposto della sposa e rose bianche",
		eng: "Bride's place card and white roses",
	},
	{ it: "Cartellino del segnaposto della sposa", eng: "Bride's place card" },
	{
		it: "Allestimento con candele e rose bianche per i tavoli del ricevimento",
		eng: "Wedding floral arrangement with candles and white roses for the reception tables",
	},
	{
		it: "Allestimento con candele e rose bianche per i tavoli del ricevimento",
		eng: "Wedding floral arrangement with candles and white roses for the reception tables",
	},
	{
		it: "Occhiali da sole a forma di cuore personalizzati",
		eng: "Personalized heart-shaped sunglasses",
	},
	{
		it: "Sposi in posa per la foto con gli occhiali da sole mentre Vanessa sistema il vestito della sposa",
		eng: "Bride and groom posing for a photo with sunglasses while Vanessa adjusts the bride's dress",
	},
	{
		it: "Sposi in posa per la foto con gli occhiali da sole",
		eng: "Bride and groom posing for a photo with sunglasses",
	},
	{
		it: "Sposa riflessa nello specchio personalizzato",
		eng: "Bride reflected in the personalized mirror",
	},
	{
		it: "Sposi che si specchiano nello specchio personalizzato",
		eng: "Bride and groom reflected in the personalized mirror",
	},
	{
		it: "Damigelle d'onore in posa con lo specchio personalizzato",
		eng: "Bridesmaids posing with the personalized mirror",
	},
	{
		it: "Tavolo dell'album degli ospiti con occhiali da sole a forma di cuore e cartello con scritto 'Scatta una foto'",
		eng: "Guest book table with heart-shaped sunglasses and sign saying 'Take a photo'",
	},
	{
		it: "Cartello con scritto 'Scatta una foto'",
		eng: "Sign saying 'Take a photo'",
	},
	{
		it: "Tavoli del ricevimento apparecchiati con candele e rose rosa",
		eng: "Reception tables set with candles and pink roses",
	},
	{
		it: "Tavoli del ricevimento apparecchiati con candele e rose rosa",
		eng: "Reception tables set with candles and pink roses",
	},
	{
		it: "Vanessa e la sua assistente in posa davanti al pannello con scritto 'Trova il tuo posto'",
		eng: "Vanessa and her assistant posing in front of the panel saying 'Find your seat'",
	},
	{
		it: "Vanessa e la sua assistente in posa davanti ai tavoli apparecchiati",
		eng: "Vanessa and her assistant posing in front of the set reception tables",
	},
	{
		it: "Vanessa riflessa nello specchio personalizzato",
		eng: "Vanessa reflected in the personalized mirror",
	},
	{
		it: "Vanessa che sistema i regali per gli ospiti sul tavolo",
		eng: "Vanessa arranging the gifts for the guests on the table",
	},
	{
		it: "Gli sposi scendono le scale circondati dalle scintille delle fiaccole",
		eng: "The bride and groom descend the stairs surrounded by the sparks of the torches",
	},
	{
		it: "Tavolo della torta nuziale con bouquet di rose bianche e candelabri",
		eng: "Wedding cake table with white rose bouquet and candelabras",
	},
	{
		it: "Cerchio con mappa dei tavoli",
		eng: "Circle with table map",
	},
	{
		it: "Biscotti personalizzati con fiori e iniziali degli sposi",
		eng: "Customized cookies with flowers and the couple's initials",
	},
	{
		it: "Tavolo del buffet dei dolci con biscotti, confetti e muffin",
		eng: "Dessert buffet table with cookies, candies, and muffins",
	},
	{
		it: "Tavolo del buffet dei dolci con biscotti, confetti e muffin",
		eng: "Dessert buffet table with cookies, candies, and muffins",
	},
	{
		it: "Tavolo del buffet dei dolci con rose gialle, tende di pizzo e arco d'oro con scritto '50'",
		eng: "Dessert buffet table with yellow roses, lace curtains, and a golden arch with '50' written on it",
	},
	{
		it: "Regali per gli ospiti e cassetta per bigliettini con messaggi per gli sposi",
		eng: "Gifts for the guests and a box for notes with messages for the couple",
	},
	{
		it: "Scatolina con i nomi degli sposi e la scritta '50 anni insieme'",
		eng: "Box with the couple's names and the inscription '50 years together'",
	},
	{
		it: "Segnaposti e menù degli sposi",
		eng: "Place cards and menus for the couple",
	},
	{
		it: "Tavolo della torta nuziale decorato con un arco di fiori, candele e rose bianche e rosa",
		eng: "Wedding cake table decorated with a flower arch, candles, and white and pink roses",
	},
	{
		it: "Torta nuziale decorata con candele e rose bianche e rosa",
		eng: "Wedding cake decorated with candles and white and pink roses",
	},
	{
		it: "Torta nuziale decorata con candele e rose bianche e rosa",
		eng: "Wedding cake decorated with candles and white and pink roses",
	},
	{
		it: "Cartolina personalizzata con i nomi degli sposi",
		eng: "Personalized card with the couple's names",
	},
	{
		it: "Pacchetti dei confetti con i nomi degli sposi",
		eng: "Confetti packages with the couple's names",
	},
	{
		it: "Pacchetti dei confetti con i nomi degli sposi",
		eng: "Confetti packages with the couple's names",
	},
	{
		it: "Menu, candele e rose rosa",
		eng: "Menu, candles, and pink roses",
	},
	{
		it: "Menu, candele e rose rosa",
		eng: "Menu, candles, and pink roses",
	},
	{
		it: "Segnaposto con cartolina di ringraziamento e regalo per gli ospiti",
		eng: "Place card with thank you card and gift for the guests",
	},
	{
		it: "Cartolina di ringraziamento e cioccolatino per gli ospiti",
		eng: "Thank you card and chocolate for the guests",
	},
	{
		it: "Menu",
		eng: "Menu",
	},
	{
		it: "Cartolina personalizzata con i nomi degli sposi",
		eng: "Personalized card with the couple's names",
	},
];
const genderAltTexts = [
	{
		it: "Biscotti, bocce di confetti e palloncini rosa e azzurri",
		eng: "Pink and blue cookies, candy bowls, and balloons",
	},
	{
		it: "Biscotti a forma di tutina rosa e azzurra",
		eng: "Cookies in the shape of pink and blue pajamas",
	},
	{
		it: "Quadretto con tutine rosa e azzurra",
		eng: "Square with pink and blue pajamas",
	},
	{
		it: "Alzatina decorata con metà tovaglia rosa e metà azzurra davanti a palloncini con scritta 'Boy' e 'Girl'",
		eng: "Stand decorated with half pink and half blue tablecloth in front of 'Boy' and 'Girl' balloons",
	},
	{
		it: "Vanessa che scatta una foto all'allestimento del gender reveal",
		eng: "Vanessa taking a photo of the gender reveal setup",
	},
];
const babyAltTexts = [
	{
		it: "Tavolo di legno con dolcetti, palloncini bianchi e beige e peluche di orso",
		eng: "Wooden table with sweets, white and beige balloons, and teddy bear plush",
	},
];
const welcomeAltTexts = [
	{
		it: "Bocce di confetti con cartellino che indica i gusti",
		eng: "Candy bowls with tags indicating the flavors",
	},
	{
		it: "Cornice con le info sul bambino appena nato",
		eng: "Frame with information about the newborn baby",
	},
	{
		it: "Scritta dorata col nome del bambino",
		eng: "Golden sign with the baby's name",
	},
	{
		it: "Biscotto personalizzato con il nome del bambino e il fiocco azzurro",
		eng: "Personalized cookie with the baby's name and blue bow",
	},
	{
		it: "Biscotto a forma di pigiamino con l'iniziale del bambino",
		eng: "Onesie-shaped cookie with the baby's initial",
	},
	{
		it: "Palloncini azzurri e bianchi e arco dorato con scritto 'Benvenuto'",
		eng: "Blue and white balloons and golden arch with 'Welcome' written on it",
	},
	{
		it: "Biscotti personalizzati a forma di luna e orsetto",
		eng: "Personalized cookies in the shape of a moon and teddy bear",
	},
	{
		it: "Tavolo di benvenuto con tovagliato azzurro, nome dorato del bambino e dolcetti",
		eng: "Welcome table with blue tablecloth, golden baby's name, and sweets",
	},
	{
		it: "Tavolo di benvenuto con tovagliato azzurro, nome dorato del bambino e dolcetti davanti a un arco dorato con scritto 'Benvenuto'",
		eng: "Welcome table with blue tablecloth, golden baby's name, and sweets in front of a golden arch with 'Welcome' written on it",
	},
	{
		it: "Tavolo con fiocchi lilla e arco dorato con scritto 'Benvenuta'",
		eng: "Table with lilac bows and golden arch with 'Welcome' written on it",
	},
	{
		it: "Tavolo della torta con fiocchi lilla e arco dorato con scritto 'Benvenuta'",
		eng: "Cake table with lilac bows and golden arch with 'Welcome' written on it",
	},
	{
		it: "Biscotti personalizzati con il nome della bambina e scritta in oro rosa",
		eng: "Personalized cookies with the baby's name and rose gold name sign",
	},
	{
		it: "Bocce di confetti rosa e biscotti personalizzati con il nome della bambina",
		eng: "Pink candies bowls and personalized cookies with the baby's name",
	},
	{
		it: "Scatoline con il nome della bambina e il fiocco rosa in una scatola con il coperchio di vetro e l'impugnatura a forma di iniziale del nome",
		eng: "Boxes with the baby's name and pink bow in a glass lid box with handle in the shape of the initial of the name",
	},
	{
		it: "Dolcetti e biscotti",
		eng: "Sweets and cookies",
	},
	{
		it: "Biscotto con scarpette di danza",
		eng: "Cookie with ballet shoes",
	},
	{
		it: "Biscotto con scritto 'Eccomi'",
		eng: "Cookie with 'Here I am' written on it",
	},
	{
		it: "Biscotto a forma di fiocco rosa",
		eng: "Cookie in the shape of a pink bow",
	},
];
const baptismAltTexts = [
	{
		it: "Dolcetti e confetti a tema coniglietti",
		eng: "Bunny-themed sweets and candies",
	},
	{
		it: "Battesimo a tema coniglietti con alzatine trasparenti e cartonato di coniglietto",
		eng: "Bunny-themed baptism with transparent stands and bunny-themed card",
	},
	{
		it: "Decorazioni a tema schiaccianoci",
		eng: "Nutcracker-themed decorations",
	},
	{
		it: "Torta a forma di fiocco bianco con scritto 'Battesimo' e il nome della bambina",
		eng: "White ribbon-shaped cake with 'Baptism' and the baby's name written on it",
	},
	{
		it: "Battesimo a tema schiaccianoci con cartonati raffiguranti i personaggi della favola",
		eng: "Nutcracker-themed baptism with cards depicting the characters from the story",
	},
	{
		it: "Stanza decorata a tema schiaccianoci",
		eng: "Nutcracker-themed room decoration",
	},
	{
		it: "Cartonato principessa dello schiaccianoci",
		eng: "Princess card from the nutcracker story",
	},
	{
		it: "Torta dorata con orsacchiotto e cerchio dorato decorato con palloncini",
		eng: "Golden cake with teddy bear and golden circle decorated with balloons",
	},
	{
		it: "Battesimo a tema orsacchiotto con palloncini dorati, bianchi e verdi",
		eng: "Teddy bear-themed baptism with golden, white and green balloons",
	},
	{
		it: "Tavolo della torta a tema arcobaleno",
		eng: "Rainbow-themed cake table",
	},
	{
		it: "Torta decorata con rose rosa",
		eng: "Cake decorated with pink roses",
	},
	{
		it: "Composizione floreale con margherite e fiori lilla",
		eng: "Floral arrangement with daisies and lilac flowers",
	},
	{
		it: "Palloncini rosa, arco dorato e scritta 'Battesimo'",
		eng: "Pink balloons, golden arch and 'Baptism' sign",
	},
	{
		it: "Biscotto personalizzato con il nome della bambina",
		eng: "Personalized cookie with the baby's name",
	},
	{
		it: "Torta con orsacchiotto che tiene un palloncino",
		eng: "Cake with teddy bear holding a balloon",
	},
	{
		it: "Alzatine a forma di colonne bianche, arco dorato e palloncini rosa",
		eng: "White column-shaped stands, golden arch and pink balloons",
	},
	{
		it: "Fiore in organza rosa",
		eng: "Pink flower in tulle",
	},
	{
		it: "Fiore in organza rosa",
		eng: "Pink flower in tulle",
	},
	{
		it: "Battesimo a tema barchetta",
		eng: "Boat-themed baptism",
	},
	{
		it: "Biscotto a forma di barchetta e barchetta di carta",
		eng: "Cookie in the shape of a boat and paper boat",
	},
	{
		it: "Tavolo apparecchiato decorato con barchetta di carta e vasi con rose bianche",
		eng: "Table setting decorated with paper boat and vases with white roses",
	},
	{
		it: "Biscotti personalizzati con il nome del bambino",
		eng: "Personalized cookies with the child's name",
	},
	{
		it: "Festa di battesimo in riva al mare",
		eng: "Baptism party by the sea",
	},
	{
		it: "Candele lilla con fiocchi rosa e quadretto con il nome della bambina battezzata",
		eng: "Lilac candles with pink ribbons and frame with the baptized child's name",
	},
	{
		it: "Festa di battesimo in riva al mare",
		eng: "Baptism party by the sea",
	},
	{
		it: "Battesimo a tema fatine",
		eng: "Fairy-themed baptism",
	},
	{
		it: "Arco dorato con cartonato di fatina e palloncini rosa e bianchi",
		eng: "Golden arch with fairy-themed card and pink and white balloons",
	},
	{
		it: "Torta decorata con fiori rosa e bianchi",
		eng: "Cake decorated with pink and white flowers",
	},
	{
		it: "Alzatine a forma di colonne bianche, archi dorati e grandi fiori rosa di carta",
		eng: "White column-shaped stands, golden arches and large pink paper flowers",
	},
	{
		it: "Torta decorata con fiori rosa e bianchi",
		eng: "Cake decorated with pink and white flowers",
	},
	{
		it: "Biscotti personalizzati con il nome della bambina",
		eng: "Personalized cookies with the girl's name",
	},
	{
		it: "Biscotti personalizzati con il nome della bambina e cake-pop",
		eng: "Personalized cookies with the girl's name and cake-pop",
	},
];
const communionAltTexts = [
	{
		it: "Allestimento esterno con fiori rosa",
		eng: "Outdoor setup with pink flowers",
	},
	{
		it: "Tavolo delle bomboniere decorato con fiori rosa",
		eng: "Guests gifts table with pink flowers",
	},
	{
		it: "Cornice con il nome della bambina e la data della comunione",
		eng: "Frame with the girl's name and the date of the communion",
	},
	{
		it: "Tavola decorata con bouquet di fiori rosa",
		eng: "Table decorated with pink flower bouquets",
	},
	{
		it: "Segnaposto con angioletto di legno",
		eng: "Place card with wooden angel",
	},
	{
		it: "Segnaposto con preghiera",
		eng: "Place card with prayer",
	},
	{
		it: "Vaso di tulipani e nebbiolina",
		eng: "Vase with tulips and baby's breath",
	},
	{
		it: "Tavolo con centrino di pizzo e arco con tende blu e fiori bianchi e blu",
		eng: "Table with lace doily and arch with blue curtains and white and blue flowers",
	},
	{
		it: "Tavolo vista mare con arco e fiori bianchi e azzurri",
		eng: "Seaside table with arch and white and blue flowers",
	},
	{
		it: "Alzatine in oro rosa e fondali verde pastello",
		eng: "Rose gold stands and pastel green backdrops",
	},
	{
		it: "Segnaposto con cioccolatino",
		eng: "Place card with chocolate",
	},
	{
		it: "Vaso di paglia con bouquet di fiori lilla e rose rosa",
		eng: "Straw vase with bouquet of lilac flowers and pink roses",
	},
	{
		it: "Tavolo decorato con fiori di campo rosa e lilla e tenda di pizzo",
		eng: "Table decorated with pink and lilac wildflowers and lace tent",
	},
	{
		it: "Alzatina della torta decorata con tulle rosa",
		eng: "Cake stand decorated with pink tulle",
	},
	{
		it: "Biscotti glassati personalizzati",
		eng: "Personalized iced cookies",
	},
	{
		it: "Alzatina della torta decorata con fiori rosa",
		eng: "Cake stand decorated with pink flowers",
	},
	{
		it: "Biscotti personalizzati a forma di tulipano",
		eng: "Personalized tulip-shaped cookies",
	},
	{
		it: "Tavolo delle bomboniere decorato con nebbiolina e tende verdi",
		eng: "Guests gifts table decorated with baby's breath and green curtains",
	},
	{
		it: "Tavolo di legno decorato con archi con rampicanti e scritta 'La mia Prima Comunione'",
		eng: "Wooden table decorated with arches with climbing plants and 'My First Communion' sign",
	},
	{
		it: "Quadretto con dedica su Gesù",
		eng: "Frame with dedication to Jesus",
	},
	{
		it: "Menù con stampe floreali e nome della bambina",
		eng: "Menu with floral prints and the girl's name",
	},
	{
		it: "Segnaposto con dedica e cioccolatino",
		eng: "Place card with dedication and chocolate",
	},
	{
		it: "Alzatine della torta con nome del bambino e cerchio decorato con palloncini",
		eng: "Cake stands with the child's name and circle decorated with balloons",
	},
	{
		it: "Alzatine della torta con nome del bambino e cerchio decorato con palloncini",
		eng: "Cake stands with the child's name and circle decorated with balloons",
	},
	{
		it: "Menù personalizzato con nome del bambino",
		eng: "Personalized menu with the child's name",
	},
	{
		it: "Menù personalizzato con nome della bambina e boccette con fiori",
		eng: "Personalized menu with the girl's name and flower bottles",
	},
	{
		it: "Omaggio per gli ospiti con scatolina personalizzata con il nome della bambina",
		eng: "Gift for guests with personalized box with the girl's name",
	},
	{
		it: "Scritta 'La mia Prima Comunione' sulla torta",
		eng: "Sign 'My First Communion' on the cake",
	},
	{
		it: "Cartolina segnaposto con motivi floreali e nome della bambina",
		eng: "Place card with floral patterns and the girl's name",
	},
	{
		it: "Scritta con il nome del bambino sulla torta",
		eng: "Sign with the child's name on the cake",
	},
	{
		it: "Scritta con il nome del bambino sulla torta",
		eng: "Sign with the child's name on the cake",
	},
	{
		it: "Alzatine dorate davanti a un pannello di peonie",
		eng: "Golden stands in front of a peony panel",
	},
	{
		it: "Scritta 'La mia Prima Comunione' su un pannello di peonie",
		eng: "Sign 'My First Communion' on a peony panel",
	},
	{
		it: "Scritta 'La mia Prima Comunione' su un pannello di peonie",
		eng: "Sign 'My First Communion' on a peony panel",
	},
	{
		it: "Tavolo di legno decorato con tulle verde e nebbiolina",
		eng: "Wooden table decorated with green tulle and baby's breath",
	},
	{
		it: "Pannello di legno con ciambelle glassate appese",
		eng: "Iced donuts on a wooden panel",
	},
	{
		it: "Alzatine bianche davanti a un cerchio decorato con palloncini azzurri e rampicanti",
		eng: "White stands in front of a circle decorated with blue balloons and climbing plants",
	},
	{
		it: "Quadretto con la scritta 'La mia Prima Comunione' e piccolo cerchio decorato con fiori bianchi e nebbiolina",
		eng: "Frame with the sign 'My First Communion' and a small circle decorated with white flowers and baby's breath",
	},
];
const confirmationAltTexts = [
	{
		it: "Scritta con nome del ragazzo sulla torta",
		eng: "Sign with the boy's name on the cake",
	},
	{
		it: "Tavolo della torta decorato con piante, nebbiolina e candele",
		eng: "Cake table decorated with plants, baby's breath, and candles",
	},
	{
		it: "Tavolo della torta decorato con piante, nebbiolina e candele",
		eng: "Cake table decorated with plants, baby's breath, and candles",
	},
	{
		it: "Tavolo della torta decorato con piante, nebbiolina e candele",
		eng: "Cake table decorated with plants, baby's breath, and candles",
	},
	{
		it: "Tappeto di paglia intrecciato con bocce di vetro e piante",
		eng: "Woven straw rug with glass bowls and plants",
	},
	{
		it: "Cornice con scritto 'La vita è corta, mangia un dolce' sul tavolo del buffet di dolci",
		eng: "Frame with 'Life is short, eat a dessert' on the dessert buffet table",
	},
	{
		it: "Alzatina a tre piani con profitteroles",
		eng: "Three-tier stand with profiteroles",
	},
	{
		it: "Alzatina della torta con scritta con il nome del ragazzo e fondale con fiori rossi",
		eng: "Cake stand with the boy's name and backdrop with red flowers",
	},
	{
		it: "Tavola decorata con runner azzurro e fiori rossi",
		eng: "Table decorated with blue runner and red flowers",
	},
	{
		it: "Tavolo di legno decorato con piante e fiori bianchi davanti a vetrata che da sul giardino",
		eng: "Wooden table decorated with plants and white flowers in front of a window overlooking the garden",
	},
	{
		it: "Compoiszione di tulipani bianchi e muschio",
		eng: "Composition of white tulips and moss",
	},
	{
		it: "Menù, segnatavolo e ampolle con fiori bianchi",
		eng: "Menu, table card, and bottles with white flowers",
	},
	{
		it: "Tavolo di legno decorato con nebbiolina e fieno",
		eng: "Wooden table decorated with baby's breath and hay",
	},
	{
		it: "Fetta di legno con ampolle contenenti nebbiolina e fiori bianchi",
		eng: "Wooden slice with bottles containing baby's breath and white flowers",
	},
	{
		it: "Confettata",
		eng: "Candy table",
	},
	{
		it: "Tavolo della torta davanti a archi dorati decorati con nebbiolina e rampicanti",
		eng: "Cake table in front of golden arches decorated with baby's breath and climbing plants",
	},
	{
		it: "Fiore di carta e nebbiolina",
		eng: "Paper flower and baby's breath",
	},
];
const bdayAltTexts = [
	{
		it: "Festeggiata che spezza la candelina del diciottesimo compleanno",
		eng: "Birthday girl breaking the candle of her 18th birthday cake",
	},
	{
		it: "Nome del festeggiato e numero 18 dorati sulla torta di compleanno",
		eng: "Name of the birthday person and number 18 in gold on the birthday cake",
	},
	{
		it: "Tavola decorata con candele e fiori bianchi",
		eng: "Table decorated with candles and white flowers",
	},
	{
		it: "Scatolina regalo per gli ospiti con il nome del festeggiato e il numero 18",
		eng: "Gift box for guests with the name of the birthday person and the number 18",
	},
	{
		it: "Torre di scatoline regalo per gli ospiti con il nome del festeggiato e il numero 18",
		eng: "Tower of gift boxes for guests with the name of the birthday person and the number 18",
	},
	{
		it: "Cornice con scritto 'Eighteen' e il nome del festeggiato",
		eng: "Frame with 'Eighteen' and the name of the birthday person",
	},
	{
		it: "Fiori bianchi che escono dal cassetto di un tavolo su cui sono disposti segnaposti, ampolle di fiori bianchi e candele",
		eng: "White flowers coming out of the drawer of a table on which place cards, bottles of white flowers, and candles are arranged",
	},
	{
		it: "Fiori bianchi che escono dal cassetto di un tavolo su cui sono disposti segnaposti, ampolle di fiori bianchi e candele",
		eng: "White flowers coming out of the drawer of a table on which place cards, bottles of white flowers, and candles are arranged",
	},
	{
		it: "Stand della torta decorato con tulle rosa e fondale glitterato",
		eng: "Cake stand decorated with pink tulle and glittery backdrop",
	},
	{
		it: "Stand della torta decorato con tulle bianco, fondale glitterato e composizione di palloncini bianchi e nastrini neri",
		eng: "Cake stand decorated with white tulle, glittery backdrop, and composition of white balloons and black ribbons",
	},
	{
		it: "Stand della torta decorato con tulle bianco, fondale glitterato e composizione di palloncini bianchi e nastrini neri",
		eng: "Cake stand decorated with white tulle, glittery backdrop, and composition of white balloons and black ribbons",
	},
	{
		it: "Menù a tema carte di cuori",
		eng: "Menu with a hearts card theme",
	},
	{
		it: "Menù a tema carte di cuori sul tavolo con ampolle di vetro con rose rosse",
		eng: "Menu with a hearts card theme on the table with glass bottles with red roses",
	},
	{
		it: "Segnaposto con leccalecca a forma di cuore e fiocco nero",
		eng: "Place card with lollipop in the shape of a heart and black bow",
	},
	{
		it: "Pannello con posti a sedere avvolto da tende rosse e ampolle con candele",
		eng: "Panel with seating wrapped in red curtains and bottles with candles",
	},
	{
		it: "Salone a tema carte da poker",
		eng: "Poker-themed hall",
	},
	{
		it: "Torta di compleanno a tema carte da poker con gigantografia della carta della Regina di Picche",
		eng: "Poker-themed birthday cake with a large print of the Queen of Spades card",
	},
	{
		it: "Poster della Regina di Picche",
		eng: "Poster of the Queen of Spades",
	},
	{
		it: "Poster del Re di Quadri",
		eng: "Poster of the King of Diamonds",
	},
	{
		it: "Cartellino di Venè tra carte da gioco e dadi",
		eng: "Venè's card among playing cards and dice",
	},
	{
		it: "Torta di compleanno a tema carte da poker con fondale drappeggiato con nome del festeggiato",
		eng: "Poker-themed birthday cake with draped backdrop featuring the name of the birthday person",
	},
	{
		it: "Cornice con scritto 'Rum bar'",
		eng: "Frame with 'Rum bar' written on it",
	},
	{
		it: "Segnaposto con sigaro e fiocco nero",
		eng: "Place card with cigar and black bow",
	},
	{
		it: "Libro delle dediche degli invitati a tema viaggio",
		eng: "Guest dedication book with travel theme",
	},
	{
		it: "Segnaposti a tema viaggio",
		eng: "Travel-themed place cards",
	},
	{
		it: "Mappa dei posti a sedere a tema viaggio",
		eng: "Travel-themed seating chart",
	},
	{
		it: "Alzatine a tema vespa",
		eng: "Vespa-themed cake stands",
	},
	{
		it: "Cartonato di una vespa bianca",
		eng: "Cardboard cutout of a white Vespa",
	},
	{
		it: "Leccalecca incartato con il nome della festeggiata e fiocco nero",
		eng: "Lollipop wrapped with the birthday girl's name and black bow",
	},
	{
		it: "Sfondo per le foto a tema copertina di VOGUE",
		eng: "Photo spot with VOGUE cover theme",
	},
	{
		it: "Mini bottiglia di Molinari come segnaposto",
		eng: "Mini Molinari bottle as a place card",
	},
	{
		it: "Libro delle dediche degli invitati a tema astronomia",
		eng: "Guest dedication book with astronomy theme",
	},
	{
		it: "Sfondo per le foto con stella illuminata e palloncini blu e argento",
		eng: "Photo backdrop with illuminated star and blue and silver balloons",
	},
	{
		it: "Torre di palloncini neri e viola con nome del festeggiato e numero 18",
		eng: "Tower of black and purple balloons with the birthday person's name and number 18",
	},
	{
		it: "Alzatine per la torta con torre di palloncini e scritta neon 'Happy Birthday'",
		eng: "Cake stands with balloon tower and neon 'Happy Birthday' sign",
	},
	{
		it: "Torre di palloncini su cui poggia la torta di compleanno, alzatine e sfondo con drappeggio trasparente",
		eng: "Balloon tower supporting the birthday cake, cake stands, and backdrop with transparent drapery",
	},
	{
		it: "Numero 18 dorato su torta di compleanno con fondale arco con palloncini e scritta neon 'Happy Birthday'",
		eng: "Golden number 18 on birthday cake with balloon arch backdrop and neon 'Happy Birthday' sign",
	},
	{
		it: "Setup torta con alzatine, archi e palloncini e numeri 18 luminosi",
		eng: "Cake setup with stands, arches, balloons, and illuminated number 18",
	},
	{
		it: "Scatola per i bigliettini con le dediche degli invitati",
		eng: "Box for cards with guests' dedications",
	},
	{
		it: "Setup torta con alzatine trasparenti, palloncini dorati e scritta neon 'Happy Birthday'",
		eng: "Cake setup with transparent stands, golden balloons, and neon 'Happy Birthday' sign",
	},
	{
		it: "Setup torta per due festeggiati con fondale dorato glitterato e scritta neon 'Happy Birthday'",
		eng: "Cake setup for two birthday celebrants with glittery gold backdrop and neon 'Happy Birthday' sign",
	},
	{
		it: "Setup torta decorato con tappeto di paglia, bouquet di fieno e palloncini verdi e dorati",
		eng: "Cake setup decorated with straw mat, hay bouquet, and green and golden balloons",
	},
	{
		it: "Scritta neon 'Happy Birthday' su pannello a tasselli neri",
		eng: "Neon 'Happy Birthday' sign on black tile panel",
	},
	{
		it: "Stanza decorata con palloncini, 18 luminoso e scritta neon 'Happy Birthday'",
		eng: "Room decorated with balloons, illuminated number 18, and neon 'Happy Birthday' sign",
	},
	{
		it: "Grande fiocco blu su pannello a tasselli argentati e scritta neon 'Happy Birthday'",
		eng: "Large blue bow on silver tile panel and neon 'Happy Birthday' sign",
	},
	{
		it: "Stanza decorata con grande fiocco blu, pannello a tasselli argentati, 18 luminoso, palloncini bianchi e scritta neon 'Happy Birthday'",
		eng: "Room decorated with large blue bow, silver tile panel, illuminated number 18, white balloons, and neon 'Happy Birthday' sign",
	},
	{
		it: "Fialetta di profumo come segna posto con nome del festeggiato e numero 18",
		eng: "Fragrance bottle as a place card with the birthday person's name and number 18",
	},
	{
		it: "Maschera del Carnevale di Venezia per un diciottesimo a tema ballo in maschera",
		eng: "Venetian Carnival mask for an 18th birthday theme party",
	},
	{
		it: "Nome della festeggiata dorato e glitterato su un pannello di tasselli neri",
		eng: "Golden and glittered name of the birthday girl on a black tile panel",
	},
	{
		it: "Maschera in pizzo nero e quadretto con nome della festeggiata e numero 18",
		eng: "Black lace mask and frame featuring the birthday girl's name and the number 18",
	},
	{
		it: "Vasi con rose bianche e candele",
		eng: "Vases with white roses and candles",
	},
	{
		it: "Quadretto del tavolo dei dolci e candela con nastro nero",
		eng: "Frame of the dessert table and candle with black ribbon",
	},
	{
		it: "Tavolo dei dolci con candelabri, vasi con rose bianche e runner di tulle nero",
		eng: "Dessert table with candelabras, vases with white roses, and black tulle runner",
	},
	{
		it: "Setup torta con alzatine trasparenti, grandi piume bianche e nere e nome dorato della festeggiata",
		eng: "Cake setup with transparent stands, large white and black feathers, and golden name of the birthday girl",
	},
	{
		it: "Tavolo apparecchiato e decorato con vasi di rose bianche",
		eng: "Table set and decorated with vases of white roses",
	},
	{
		it: "Setup torta con alzatine trasparenti, arco di palloncini e lucine e scritta neon 'Happy Birthday'",
		eng: "Cake setup with transparent stands, balloon and fairy lights arch, and neon 'Happy Birthday' sign",
	},
];
const graduationAltTexts = [
	{
		it: "Arco dorato con scritta 'Laurea', lucine e drappeggio rosso",
		eng: "Golden arch with 'Graduation' sign, fairy lights and red drapery",
	},
	{
		it: "Biscotti a forma di camice e stetoscopio",
		eng: "Cookies in the shape of a lab coat and stethoscope",
	},
	{
		it: "Cofanetto con confetti rossi e biscotto con disegnata una siringa",
		eng: "Box with red confetti and cookie with a syringe drawn on it",
	},
	{
		it: "Menù su tavolo apparecchiato con tovaglia bianca e runner di tulle rosso",
		eng: "Menu on a table set with a white tablecloth and red tulle runner",
	},
	{
		it: "Tavolo decorato con runner di tulle rosso e vasi con rose rosse e candele bianche",
		eng: "Table decorated with red tulle runner and vases with red roses and white candles",
	},
	{
		it: "Tavolo decorato con runner di tulle rosso e vasi con rose rosse e candele bianche",
		eng: "Table decorated with red tulle runner and vases with red roses and white candles",
	},
	{
		it: "Segnaposto con fiocco rosso, cartellino rotondo con su scritto 'Thank you' e cartellino con il nome della laureata",
		eng: "Place card with red bow, round card with 'Thank you' written on it, and card with the graduate's name",
	},
	{
		it: "Menù con nome della laureata",
		eng: "Menu with the graduate's name",
	},
	{
		it: "Cartellini con il nome della laureata e l'indicazione del corso di studi",
		eng: "Place cards with the graduate's name and field of study",
	},
	{
		it: "Arco dorato con scritta 'Laurea', lucine e drappeggio rosso",
		eng: "Golden arch with 'Graduation' sign, fairy lights and red drapery",
	},
	{
		it: "Vanessa sistema il drappeggio rosso dell'arco",
		eng: "Vanessa arranges the red drapery of the arch",
	},
];
const partyAltTexts = [
	{
		it: "Festeggiata avvolta da una pioggia di lucine",
		eng: "Birthday girl surrounded by a shower of fairy lights",
	},
	{
		it: "Festeggiata avvolta da una pioggia di lucine",
		eng: "Birthday girl surrounded by a shower of fairy lights",
	},
	{
		it: "Composizione con nome della festeggiata, numero 30, palloncini e fiori",
		eng: "Composition with the birthday girl's name, number 30, balloons, and flowers",
	},
	{
		it: "Composizione con nome della festeggiata, numero 30, palloncini e fiori",
		eng: "Composition with the birthday girl's name, number 30, balloons, and flowers",
	},
	{
		it: "Setup torta e foto per party in piscina con torre di palloncini, fiori e scatola per le dediche degli invitati",
		eng: "Cake and photo setup for pool party with balloon tower, flowers, and guest dedication box",
	},
	{
		it: "Tavoli del ricevimento del 30esimo compleanno con setup torta sullo sfondo",
		eng: "Reception tables for the 30th birthday with cake setup in the background",
	},
	{
		it: "Vanessa sistema la composizione floreale",
		eng: "Vanessa arranges the floral composition",
	},
	{
		it: "Composizione di fiori viola, lilla e rosa",
		eng: "Composition of purple, lilac, and pink flowers",
	},
	{
		it: "Vanessa sistema i palloncini alla base della torre",
		eng: "Vanessa arranges the balloons at the base of the tower",
	},
	{
		it: "Torta del 30esimo compleanno decorata con numero, nome e piume rosa",
		eng: "30th birthday cake decorated with number, name, and pink feathers",
	},
	{
		it: "Vanessa sistema un fiore rosa nella composizione",
		eng: "Vanessa arranges a pink flower in the composition",
	},
	{
		it: "Un grosso fiore rosa spicca al centro della composizione floreale",
		eng: "A large pink flower stands out at the center of the floral arrangement",
	},
	{
		it: "Vanessa appende il nome della festeggiata al supporto",
		eng: "Vanessa hangs the birthday girl's name on the support",
	},
	{
		it: "Composizione con nome della festeggiata, numero 30, palloncini e fiori",
		eng: "Composition with the birthday girl's name, number 30, balloons, and flowers",
	},
	{
		it: "Vanessa scatta una foto alla composizione ultimata",
		eng: "Vanessa takes a photo of the completed arrangement",
	},
	{
		it: "La festeggiata posa accanto alla composizione floreale",
		eng: "Birthday girl poses next to the floral arrangement",
	},
	{
		it: "Segnaposto con fialetta di profumo personalizzata",
		eng: "Place card with personalized perfume vial",
	},
	{
		it: "Insegna luminosa con scritto 'Happy Birthday' su tenda lilla e fiori rosa",
		eng: "Neon sign with 'Happy Birthday' on lilac curtain and pink flowers",
	},
	{
		it: "Gisarole giallo e nome della festeggiata sulla torta di compleanno",
		eng: "Yellow sunflower and the birthday girl's name on the birthday cake",
	},
	{
		it: "Tavolo della torta decorato con fiori gialli e arancioni",
		eng: "Cake table decorated with yellow and orange flowers",
	},
	{
		it: "Tavolo della torta decorato con fiori gialli e arancioni",
		eng: "Cake table decorated with yellow and orange flowers",
	},
	{
		it: "Torta a tema anni 50",
		eng: "50s themed cake",
	},
	{
		it: "Torta a tema anni 50 con cartonato per foto",
		eng: "50s themed cake with photo cutout",
	},
	{
		it: "Torta a tema anni 50 con cartonato per foto",
		eng: "50s themed cake with photo cutout",
	},
	{
		it: "Insegna neon con scritto 'Happy Birthday' su tenda a frange dorate",
		eng: "Neon sign with 'Happy Birthday' on gold fringe curtain",
	},
	{
		it: "Nome della festeggiata, numero 60 e fiori viola sulla torta di compleanno",
		eng: "Birthday girl's name, number 60, and purple flowers on the birthday cake",
	},
	{
		it: "Setup torta per un primo compleanno in giardino ",
		eng: "Cake setup for a first birthday in the garden",
	},
	{
		it: "Arco dorato con nome della festeggiata, palloncini e fiocchi rosa dietro all'alzatina con la torta",
		eng: "Golden arch with birthday girl's name, balloons, and pink bows behind the cake stand",
	},
	{
		it: "Fondale oro glitterato con torre di palloncini verde scuro dietro all'alzatina con la torta",
		eng: "Glittery gold backdrop with dark green balloon tower behind the cake stand",
	},
	{
		it: "Torta del primo compleanno",
		eng: "First birthday cake",
	},
	{
		it: "Setup con alzatine e cerchio di palloncini rosa, bianchi e lilla",
		eng: "Setup with cake stands and a circle of pink, white, and lilac balloons",
	},
	{
		it: "Setup con alzatine e cerchio di palloncini rosa, bianchi e lilla",
		eng: "Setup with cake stands and a circle of pink, white, and lilac balloons",
	},
	{
		it: "Palloncini, fiori e tulle intorno alla torta del primo compleanno",
		eng: "Balloons, flowers, and tulle around the first birthday cake",
	},
	{
		it: "Grosse farfalle, archi e palloncini decorano il tavolo della confettata",
		eng: "Large butterflies, arches, and balloons decorate the candy table",
	},
	{
		it: "Composizione di farfalle e palloncini avvolta in una luce blu vista dall'esterno del locale",
		eng: "Composition of butterflies and balloons wrapped in blue light seen from outside the venue",
	},
	{
		it: "Tavolo bianco con drappeggio nero davanti a un fondale con drappeggi neri e argento",
		eng: "White table with black drapery in front of a backdrop with black and silver drapery",
	},
	{
		it: "Scritta 'Hello 40' sulla torta del festeggiato",
		eng: "'Hello 40' written on the birthday cake",
	},
	{
		it: "Composizione a tema notte con stelle, lune, lucine e palloncini a bordo piscina",
		eng: "Night-themed composition with stars, moons, fairy lights, and balloons by the pool",
	},
	{
		it: "Tavolo a tema Inter per un 80esimo compleanno",
		eng: "Inter-themed table for an 80th birthday",
	},
	{
		it: "Muffin personalizzato con orsetto per un primo compleanno",
		eng: "Customized muffin with teddy bear for a first birthday",
	},
	{
		it: "Sacchettini di confetti per gli invitati chiusi con fiocchetti azzurri",
		eng: "Candy bags for guests tied with blue ribbons",
	},
	{
		it: "Sfondo a tema Ariel dietro alla torta del primo compleanno",
		eng: "Ariel-themed backdrop behind the first birthday cake",
	},
	{
		it: "Fiore rosa e boccetta di confetti",
		eng: "Pink flower and confetti bottle",
	},
	{
		it: "Tavolo della torta con tovaglia azzurra e fiori rosa",
		eng: "Cake table with blue tablecloth and pink flowers",
	},
	{
		it: "Vasetti con fiori rosa e candele lilla",
		eng: "Vases with pink flowers and lilac candles",
	},
	{
		it: "Torta con fiori rosa, numero 40 e nome della festeggiata",
		eng: "Cake with pink flowers, the number 40, and the celebrant's name",
	},
	{
		it: "Fiori rosa e candele viola decorano la torta di compleanno",
		eng: "Pink flowers and purple candles decorate the birthday cake",
	},
	{
		it: "Scenografia a tema unicorno per un primo compleanno",
		eng: "Unicorn-themed setup for a first birthday",
	},
	{
		it: "Lettere illuminate che compongono la parola ONE, nuvolette, stelle e palloncini con coriandoli",
		eng: "Illuminated letters forming the word ONE, clouds, stars, and balloons with confetti",
	},
	{
		it: "Setup torta a tema Inside Out per un sesto compleanno",
		eng: "Inside Out-themed cake setup for a sixth birthday",
	},
	{
		it: "Arco dorato con scritta neon 'Happy Birthday' e composizione di palloncini colorati dietro ai muffin a tema Inside Out",
		eng: "Golden arch with neon 'Happy Birthday' sign and colorful balloon arrangement behind Inside Out-themed muffins",
	},
	{
		it: "Muffin a tema Inside Out",
		eng: "Inside Out-themed muffins",
	},
	{
		it: "Biscotti a tema Inside Out",
		eng: "Inside Out-themed cookies",
	},
	{
		it: "Biscotti a tema Inside Out",
		eng: "Inside Out-themed cookies",
	},
	{
		it: "Portachiavi personalizzato con nome della bambina e numero 1",
		eng: "Customized keychain with the girl's name and number 1",
	},
	{
		it: "Torta del primo compleanno a tema margherite",
		eng: "First birthday cake with daisy theme",
	},
	{
		it: "Grande margherita davanti all'alzatina con la torta",
		eng: "Large daisy in front of the cake stand with the cake",
	},
	{
		it: "Caramelle a forma di margherita",
		eng: "Daisy-shaped candies",
	},
	{
		it: "Caramelle a forma di margherita e portachiavi personalizzato",
		eng: "Customized daisy-shaped candies and keychains",
	},
	{
		it: "Sacchettini dei confetti a tema Minnie",
		eng: "Minnie-themed candy bags",
	},
	{
		it: "Tavolo della torta a tema circo",
		eng: "Circus-themed cake table",
	},
	{
		it: "Torta a tema circo",
		eng: "Circus-themed cake",
	},
	{
		it: "Tavolo della torta decorato con rose arancioni e merletti bianchi",
		eng: "Cake table decorated with orange roses and white lace",
	},
	{
		it: "Alzatina trasparente davanti a un fondale nero e arco di palloncini neri e dorati",
		eng: "Transparent cake stand in front of a black backdrop and a arch of black and golden balloons",
	},
	{
		it: "Tavolo della torta con tovagliato bianco e dorato e arco con numero 50",
		eng: "Cake table with white and golden tablecloth and arch with the number 50",
	},
	{
		it: "Scenografia per un primo compleanno a tema safari",
		eng: "Safari-themed setup for a first birthday",
	},
	{
		it: "Scenografia per un primo compleanno a tema safari",
		eng: "Safari-themed setup for a first birthday",
	},
	{
		it: "Torre di palloncini bianchi con tucano sulla cima",
		eng: "Tower of white balloons with a toucan on top",
	},
	{
		it: "Tavolo di legno con merletti bianchi, fiori rosa e candelabro",
		eng: "Wooden table with white lace, pink flowers, and candelabra",
	},
	{
		it: "Vaso in ottone con fiori rosa e ulivo",
		eng: "Bronze vase with pink flowers and olive branches",
	},
	{
		it: "Quadretto con il nome della festeggiata e rose lilla",
		eng: "Picture with the celebrant's name and lilac roses",
	},
	{
		it: "Setup torta per un primo compleanno in giardino ",
		eng: "Cake setup for a first birthday in the garden",
	},
	{
		it: "Compleanno a tema PokèMon",
		eng: "PokèMon-themed birthday party",
	},
	{
		it: "Scenografia per un compleanno in riva al mare",
		eng: "Setup for a birthday by the sea",
	},
	{
		it: "Orsacchiotto gigante su una vespa azzurra",
		eng: "Giant teddy bear on a blue scooter",
	},
	{
		it: "Scenografia a tema bosco per un compleanno in una grotta",
		eng: "Forest-themed setup for a birthday in a cave",
	},
	{
		it: "Vanessa scatta una foto alla scenografia a tema bosco",
		eng: "Vanessa takes a photo of the forest-themed setup",
	},
	{
		it: "Tavolo con tovagliato rosa, fiori rosa e arco dorato con scritta luminosa 'Happy Birthday'",
		eng: "Table with pink tablecloth, pink flowers, and golden arch with luminous 'Happy Birthday' sign",
	},
	{
		it: "Fiori rosa",
		eng: "Pink flowers",
	},
	{
		it: "Tavolo di legno con tovaglia di merletto bianco e cerchio decorato con fiori e merletti bianchi",
		eng: "Wooden table with white lace tablecloth and a circle decorated with flowers and white lace",
	},
	{
		it: "Primo compleanno a tema orsacchiotti",
		eng: "First birthday party with teddy bear theme",
	},
	{
		it: "Cartonato di un panda per la cerimonia di cambio cintura di una palestra di judo",
		eng: "Panda cutout for the belt change ceremony at a judo gym",
	},
	{
		it: "Scenografia a tema api",
		eng: "Bee-themed setup",
	},
	{
		it: "Arco di cartone con sole, palloncini e nebbiolina rosa",
		eng: "Cardboard arch with sun, balloons, and pink baby-breath",
	},
	{
		it: "Torta del primo compleanno con sole",
		eng: "First birthday cake with sun",
	},
];
const showcaseAltTexts = [
	{
		it: "Vetrina di un fotografo decorata con cerchi dorati e nebbiolina",
		eng: "Photographer's showcase decorated with golden rings and baby-breath",
	},
	{
		it: "Vetrina di un fotografo decorata con cerchi dorati e nebbiolina",
		eng: "Photographer's showcase decorated with golden rings and baby-breath",
	},
	{
		it: "Vetrina natalizia con renne dorate e pacchi regalo luminosi",
		eng: "Christmas showcase with golden reindeer and glowing gift boxes",
	},
	{
		it: "Vetrina natalizia con tavolo bianco, renne dorate, alberi di natale e palla di vetro",
		eng: "Christmas showcase with white table, golden reindeer, Christmas trees, and glass ball",
	},
	{
		it: "Vetrina natalizia di un negozio di articoli per la casa",
		eng: "Christmas showcase of a home goods store",
	},
	{
		it: "Vanessa appende una grossa ghirlanda fuori da un negozio",
		eng: "Vanessa hangs a large wreath outside a store",
	},
	{
		it: "Tre ghirlande natalizie decorano la vetrina del negozio",
		eng: "Three Christmas wreaths decorate the store window",
	},
	{
		it: "Espositore con gli articoli natalizi in vendite e insegna decorata con fiocchetti rossi",
		eng: "Display with Christmas items for sale and shop sign decorated with red bows",
	},
	{
		it: "Espositore con cartello a tema schiaccianoci",
		eng: "Display with Nutcracker-themed sign",
	},
	{
		it: "Vetrina di un negozio decorata con renne luminose, ghirlande e stelle di natale",
		eng: "Store window decorated with illuminated reindeer, wreaths, and Christmas stars",
	},
	{
		it: "Vetrina natalizia con albero di Natale",
		eng: "Christmas showcase with Christmas tree",
	},
	{
		it: "Vetrina natalizia con casetta di Babbo Natale",
		eng: "Christmas showcase with Santa's house",
	},
	{
		it: "Composizione floreale con margherite in una vetrina di un negozio d'abbigliamento",
		eng: "Floral arrangement with daisies in a clothing store window",
	},
	{
		it: "Grandi margherite di carta decorano la vetrina di un negozio di abbigliamento",
		eng: "Large paper daisies decorate the window of a clothing store",
	},
];

const createPhotoSources = (src) => ({
	src,
	fullSrc: src,
	slideSrc: src.replace("/img/", "/img-optimized/slides/"),
	thumbSrc: src.replace("/img/", "/img-optimized/thumbs/"),
});

const createAlbumPhotos = (length, srcFactory, altTexts, fallbackAlt) =>
	Array.from({ length }, (_, i) => ({
		id: i + 1,
		...createPhotoSources(srcFactory(i)),
		alt: altTexts[i] || fallbackAlt,
	}));

const weddingPhotos = createAlbumPhotos(
	42,
	(i) => `/img/wedding/wedding-${i + 1}.jpg`,
	weddingAltTexts,
	{ it: "Wedding", eng: "Wedding" }
);

const genderPhotos = createAlbumPhotos(
	5,
	(i) => `/img/gender/gender-${i + 1}.jpg`,
	genderAltTexts,
	{ it: "Gender Reveal", eng: "Gender Reveal" }
);

const babyPhotos = createAlbumPhotos(
	1,
	(i) => `/img/baby/baby-${i + 1}.jpg`,
	babyAltTexts,
	{ it: "Baby Shower", eng: "Baby Shower" }
);

const welcomePhotos = createAlbumPhotos(
	18,
	(i) => `/img/welcome/welcome-${i + 1}.jpg`,
	welcomeAltTexts,
	{ it: "Welcome Home", eng: "Welcome Home" }
);

const baptismPhotos = createAlbumPhotos(
	32,
	(i) => `/img/baptism/baptism-${i + 1}.jpg`,
	baptismAltTexts,
	{ it: "Battesimo", eng: "Baptism" }
);

const communionPhotos = createAlbumPhotos(
	38,
	(i) => `/img/comunioni/comunioni-${i + 1}.jpg`,
	communionAltTexts,
	{ it: "Prima Comunione", eng: "First Communion" }
);

const confirmationPhotos = createAlbumPhotos(
	17,
	(i) => `/img/cresime/cresime-${i + 1}.jpg`,
	confirmationAltTexts,
	{ it: "Cresima", eng: "Confirmation" }
);

const bdayPhotos = createAlbumPhotos(
	56,
	(i) => `/img/18esimi/18esimo-${i + 1}.jpg`,
	bdayAltTexts,
	{ it: "Diciottesimo Compleanno", eng: "18th Birthday" }
);

const graduationPhotos = createAlbumPhotos(
	11,
	(i) => `/img/graduation/graduation-${i + 1}.jpg`,
	graduationAltTexts,
	{ it: "Laurea", eng: "Graduation" }
);

const partyPhotos = createAlbumPhotos(
	84,
	(i) => `/img/party/party-${i + 1}.jpg`,
	partyAltTexts,
	{ it: "Festa", eng: "Party" }
);

const showcasePhotos = createAlbumPhotos(
	14,
	(i) => `/img/vetrine/vetrine-${i + 1}.jpg`,
	showcaseAltTexts,
	{ it: "Vetrina", eng: "Showcase" }
);

export const albumData = {
	wedding: {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "WEDDING",
			eng: "WEDDING",
		},
		photos: weddingPhotos,
	},
	"gender-reveal": {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "GENDER REVEAL",
			eng: "GENDER REVEAL",
		},
		photos: genderPhotos,
	},

	"baby-shower": {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "BABY SHOWER",
			eng: "BABY SHOWER",
		},
		photos: babyPhotos,
	},
	"welcome-home": {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "WELCOME HOME",
			eng: "WELCOME HOME",
		},
		photos: welcomePhotos,
	},
	baptism: {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "BATTESIMO",
			eng: "BAPTISM",
		},
		photos: baptismPhotos,
	},
	communion: {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "PRIMA COMUNIONE",
			eng: "FIRST COMMUNION",
		},
		photos: communionPhotos,
	},
	confirmation: {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "CRESIMA",
			eng: "CONFIRMATION",
		},
		photos: confirmationPhotos,
	},
	"18th-birthday": {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "DICIOTTESIMO COMPLEANNO",
			eng: "18TH BIRTHDAY",
		},
		photos: bdayPhotos,
	},
	graduation: {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "LAUREA",
			eng: "GRADUATION",
		},
		photos: graduationPhotos,
	},
	party: {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "PARTY",
			eng: "PARTY",
		},
		photos: partyPhotos,
	},
	showcase: {
		helmetData: {
			title: "Album | Vanè Wedding & Event",
		},
		title: {
			it: "VETRINE",
			eng: "SHOWCASE",
		},
		photos: showcasePhotos,
	},
};
